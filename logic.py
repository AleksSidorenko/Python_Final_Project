...
#     for idx, movie in enumerate(results):
#         table.add_row(
#             str(start_index + idx + 1),  # 👈 ПРАВКА: учитываем начальный индекс
#             movie.get("title", "N/A"),
#             movie.get("description", "N/A"),
#             str(movie.get("release_year", "N/A")),
#             str(movie.get("length", "N/A")) + " мин",
...
#                 page -= 1
#         elif selected_action == "Выбрать фильм по номеру":
#             number_input = questionary.text(
#                 message="Введите номер фильма (по отображенному списку):",
#                 qmark=""
#             ).ask()
#
#             if number_input and number_input.isdigit():
#                 number = int(number_input)
#
#                 # 🔥 Проверка: номер должен быть в пределах отображаемого диапазона
#                 if start < number <= start + len(page_movies):
#                     selected_movie = results[number - 1]  # выбираем из общего списка результатов
#                     console.clear()
#                     display_movie_details(selected_movie)
#                     show_error_and_wait('')
#                 else:
#                     show_error_and_wait(
#                         "❗ Неверный номер фильма.\n[bold yellow]Пожалуйста, выберите номер из отображаемого списка.[/bold yellow]")
#             else:
#                 show_error_and_wait(
#                     "❗ Неверный номер фильма.\n[bold yellow]Пожалуйста, выберите номер из отображаемого списка.[/bold yellow]")
#         else:
...
#             if not keyword:
#                 console.print("[bold green]⏎ Нажмите Enter для возврата к списку[/bold green]")
#                 keyword = questionary.text("Ключевое слово не может быть пустым", qmark="").ask()
#                 continue
#
#             keyword = keyword.strip()
#
...
#             ).ask()
#             if choice == "🔁 Попробовать снова":
#                 continue  #  Вернуться к выбору способа ввода
#             else:
#                 fake_clear()
#                 return
#         else:
#             break  # Если фильмы найдены — выйти из цикла
#
#     paginate_results(results, keyword)
#
#
# def search_by_genre_and_year():
#     """Поиск фильмов по жанру и году с постраничным выводом."""
#     genres = fetch_all_genres()
#     if not genres:
#         console.print("[bold red]Нет доступных жанров.[/bold red]")
#         return
...
#     min_year, max_year = get_year_range_from_db()
#
#     # Цикл до корректного ввода года или диапазона
#     while True:
#         year_input = questionary.text(
#             f"Введите год или диапазон годов ({min_year}-{max_year}):", qmark=''
#         ).ask()
#
#         if not year_input:
#             console.print("[bold red]Ввод пустой. Повторите попытку.[/bold red]")
...
#
# def display_top_queries_and_select():
#     """Показать топ-5 популярных ключевых слов и выбрать одно"""
#     top_queries = get_top_queries(by_type=False)
#     if not top_queries:
#         console.print("[bold red]Нет популярных слов для отображения![/bold red]")
#         return
#     console.print("\n[bold blue]Топ-5 популярных ключевых запросов:[/bold blue]")
#
#     # Проходим по топ-5 запросов и выводим корректную информацию
#     for idx, query in enumerate(top_queries, start=1):
#         query_text = query.get('query_text', 'Неизвестный запрос')  # Получаем текст запроса
#         query_count = query.get('amount', 0)  # Получаем количество запросов
#         console.print(f"{idx}. {query_text} ({query_count} раз)")  # Выводим информацию о запросе и количестве
#
#
#
# def display_top_genre_queries_and_select():
#     """Показать топ-5 популярных жанров и выбрать один"""
#     # Получаем топ-5 запросов
#     top_genres = get_top_queries(by_type=True)
#
...
#     console.print("\n[bold blue]Топ-5 популярных жанров:[/bold blue]")
#
#     # Проходим по топ-5 жанров и выводим корректную информацию
#     for idx, query in enumerate(top_genres, start=1):
#         genre_name = query.get('query_type', 'Неизвестный жанр')  # Получаем название жанра
#         genre_count = query.get('amount_genre', 0)  # Получаем количество запросов
#         console.print(f"{idx}. {genre_name} ({genre_count} раз)")  # Выводим информацию о жанре и количестве
#
