...
def get_movies_by_keyword(keyword):
    ...
    try:
        ...
        SELECT * 
        FROM film 
        WHERE title REGEXP '\\b{keyword}\\b' 
           OR description REGEXP '\\b{keyword}\\b'
        """
        cursor.execute(query)
        results = cursor.fetchall()
        return results
    except Error as e:
        console.print(f"[red]Ошибка при поиске по ключевому слову: {e}[/red]")
        return []
    finally:
        if connection.is_connected():
            cursor.close()
...
def fetch_movies_by_keyword(keyword):
    ...
    try:
        ...
        console.print(f"[red]Ошибка при расширенном поиске по ключевому слову: {e}[/red]")
        return []
    finally:
...
def search_movies_by_genre(genre_name):
    ...
    try:
        ...
        console.print(f"[red]Ошибка при поиске по жанру: {e}[/red]")
        return []
    finally:
        if connection.is_connected():
            cursor.close()
...
def save_search_query(query, query_type=None):
    """Сохраняет поисковые запросы в лог"""
    try:
        if not query:
        ...
        console.print(f"[red]Ошибка при сохранении запроса: {e}[/red]")
    finally:
        if connection.is_connected():
            cursor.close()
            connection.close()
...
def get_top_queries(by_type=True):
    ...
    try:
        ...
        console.print(f"[red]Ошибка при получении популярных запросов: {e}[/red]")
        return []
...
def get_popular_queries():
    """Подсчёт количества одинаковых ключевых слов"""
    try:
        ...
        console.print(f"[red]Ошибка при подсчёте популярных запросов: {e}[/red]")
        return []
    finally:
        if connection.is_connected():
            cursor.close()
...
def fetch_all_keyword():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT query_text FROM query_log ORDER BY query_text")
        return [row[0] for row in cursor.fetchall()]
    except Error as e:
        console.print(f"[red]Ошибка при получении ключевых слов: {e}[/red]")
        return []
    finally:
        if conn.is_connected():
            cursor.close()
...
def fetch_all_genres():
    ...
    try:
        ...
        console.print(f"[red]Ошибка при получении жанров: {e}[/red]")
        return []
    finally:
        if conn.is_connected():
            conn.close()
...
def get_year_range_from_db():
    """Мин и макс года выпуска фильмов"""
    query = "SELECT MIN(release_year) AS min_year, MAX(release_year) AS max_year FROM film"
    result = execute_query(query, fetch_one=True)
    return result["min_year"], result["max_year"] if result else (None, None)
